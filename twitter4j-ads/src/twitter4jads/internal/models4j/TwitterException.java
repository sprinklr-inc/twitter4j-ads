...
public class TwitterException extends Exception implements TwitterResponse, HttpResponseCode {
    ...
    public int getRetryAfter() {
        int retryAfter = -1;
        if (this.statusCode == BAD_REQUEST || this.statusCode == TOO_MANY_REQUESTS) {
    ...
    public boolean exceededRateLimitation() {
        return (statusCode == 400 && getRateLimitStatus() != null && getRateLimitStatus().getRemaining() < 1) // REST API
    ...
    private final static String[] FILTER = new String[]{"twitter4jads"};
    ...
    public String toString() {
        return getMessage() + (nested ? "" : "\nRelevant discussions can be found on the Internet at:\n"
                ...
                ", headers= " + ((response != null) ? response.getResponseHeaderFields() : Collections.emptyMap()) +
                '}';
    }
